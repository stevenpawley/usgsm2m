...
* `$select_products()` and `$select_bands()` now warn when a pattern matches
  nothing, listing what was available. Product names are not standardised
  across the catalogue — the bundle is "Landsat Collection 2 Level-2 Product
  Bundle" for `landsat_ot_c2_l2` but "Standard Format" for `corona2` — so a
  pattern carried over from another dataset used to select an empty set
