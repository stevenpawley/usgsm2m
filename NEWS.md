...
* The package is now called `usgsm2m`, renamed from `USGSm2m`. The old name
  read as though the package were issued by the USGS, which it is not. Update
  `library()` calls, and reinstall from
  <https://github.com/stevenpawley/usgsm2m>.

  Scene list ids generated by the package are now prefixed `usgsm2m_` rather
  than `USGSm2m_`. Lists created under the old prefix are unaffected and can
  still be retrieved by passing their id to `M2MSession$scene_list()`.
