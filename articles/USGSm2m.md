...
others have to be staged first. Note that the API’s own `$available` and
`$requested` buckets do not map cleanly onto that — proxied downloads
sit in `$requested` but already carry a working URL — so use `$ready()`
and `$pending()`. `$is_ready()` tells you whether anything is still
