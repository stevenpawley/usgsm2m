...
Behind the scenes this registers a private *scene list* on the M2M
server, which the API requires before it will list download options. The
package owns that plumbing; the search interface remains focused on
scenes and products.
...
a subset. `$select_bands()` matches literal substrings of `displayId`,
which is how `"B4"` picks out `..._SR_B4`, and `$filter()` accepts
arbitrary `dplyr` expressions. Both return a new object, leaving the
original selection intact:
...
That name is matched **exactly**, against `productName` or
`productCode`, so it is a value copied straight out of the listing
above. Names are not standardised across the catalogue — the bundle is
“Landsat Collection 2 Level-1 Product Bundle” here and “Standard Format”
for `corona2` — and they contain characters like parentheses that
pattern matching would misread, so read them off `$scene_products()`
rather than guessing. A value matching nothing warns and lists what was
available.

For looser selection, chain `$filter()` with any expression you like:

``` r

opts$select_products()$filter(grepl("Browse", productName))
```
...
A file is ready once it has a URL. Some come straight back that way;
others have to be staged first. The API’s internal available and
requested buckets do not map cleanly onto that — proxied downloads can
be in the requested bucket but already carry a working URL — so use
`$ready()` and `$pending()`. `$is_ready()` tells you whether anything is
still staging, and `$refresh()` re-polls:
